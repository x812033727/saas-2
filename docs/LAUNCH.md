...
記帳（`ctx.add_cost`）。旗艦引擎（Ti 多專家工作坊）接入中；offline 引擎
就是 protocol 的參考實作。
...
- [x] demo GIF（dashboard 五景導覽）放 README 頂部；發文前可再照 90 秒腳本錄一版帶滑鼠操作的（vhs / LICEcap）
- [ ] Show HN 發文 + 同步 X/Reddit r/LocalLLaMA（改寫成社群語氣）（→使用者）
